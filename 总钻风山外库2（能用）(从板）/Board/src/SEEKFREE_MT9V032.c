...
int16 MT9V032_CFG[CONFIG_FINISH][2]=
{
    ...
    {FPS,               80},  //图像帧率          摄像头收到后会自动计算出最大FPS，如果过大则设置为计算出来的最大FPS
    ...
    {GAIN,             60},  //图像增益          范围16-64     增益可以在曝光时间固定的情况下改变图像亮暗程度
...
void MT9V032_camera_init(void)
{
    ...
    //uart_tx_irq_en(MT9V032_COF_UART);
...
void row_finished(void)
{
	...
	now_row++;
	if(now_row<ROW) dma_repeat(DMA_CH0,(void *)&MT9V032_DATAPORT,(void *)image[now_row],COL);
	if(now_row >= ROW)	
